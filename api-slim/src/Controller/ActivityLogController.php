...
final class ActivityLogController
{
  ...
  {
    try {
      $result = $this->activityLogService->getAll();
  ...
  {
    try {
      $result = $this->activityLogService->getOne((string) $args['id']);
      return $response->withJson($result);
    } catch (Exception $e) {
  ...
  public function create(Request $request, Response $response, array $args): Response
  {
    try {
      ...
      $validator = v::key('userId', v::optional(v::stringType()))
       ->key('action', v::stringType())
       ->key('details', v::optional(v::stringType()))
       ->key('ipAddress', v::optional(v::stringType()))
       ->key('userAgent', v::optional(v::stringType()));
  ...
  'userAgent' => $input['userAgent'],
     ...
     $this->activityLogService->create($dto);
  ...
  public function update(Request $request, Response $response, array $args): Response
  {
    try {
      $input = $request->getParsedBody();
      $validator = v::key('userId', v::optional(v::stringType()))
       ->key('action', v::optional(v::stringType()))
       ->key('details', v::optional(v::stringType()))
       ->key('ipAddress', v::optional(v::stringType()))
       ->key('userAgent', v::optional(v::stringType()));
  ...
  'userAgent' => $input['userAgent'] ?? '',
      ...
      $this->activityLogService->update((string) $args['id'], $dto);
  ...
  {
   // try {
   //   $result = $this->activityLogService->delete((string) $args['id']);
   //   return $response->withJson($result);
   // } catch (Exception $e) {
   //   return $response->withJson(['error' => $e->getMessage()], 400);
   // }
