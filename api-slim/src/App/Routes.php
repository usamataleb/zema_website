...
return static function (App $app) {
  ...
  $app->group('/images', function ($app) use ($authMiddleware, $rateLimitMiddleware) {
    ...
    $app->get('', [ImageController::class, 'getAll']);
  ...
  $app->group('/carousel', function ($app) use ($authMiddleware, $rateLimitMiddleware) {
    $app->get('', [CarouselItemController::class, 'getAll']);
  ...
  // --------------- Attachments Routes ---------------- //
  $app->group('/attachments', function ($app) use ($authMiddleware) {
    $attachments = 'App\Controller\AttachmentsController:';

    $app->get('', "{$attachments}getAll");
    $app->get('/type/{type}', "{$attachments}getByType");
    $app->post('', "{$attachments}create")->add($authMiddleware);
    $app->get('/{id}', "{$attachments}getOne")->add($authMiddleware);
    $app->put('/{id}', "{$attachments}update")->add($authMiddleware);
    $app->delete('/{id}', "{$attachments}delete")->add($authMiddleware);
  });
